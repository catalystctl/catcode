    ...
    return (
      <>
        ...
        <button
          ...
          title="Attach image"
          className="flex h-9 w-9 min-h-11 min-w-11 shrink-0 items-center justify-center rounded-sm text-ink-400 transition-colors hover:bg-ink-800 hover:text-ink-100 disabled:opacity-50 sm:min-h-0 sm:min-w-0"
        >
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      {images.map((src, i) => (
        <div key={i} className="group/img relative h-12 w-12">
          ...
          <button
            ...
            className="absolute -right-1.5 -top-1.5 flex h-6 w-6 min-h-11 min-w-11 items-center justify-center rounded-sm border border-ink-700 bg-ink-900 text-ink-300 opacity-100 transition-colors hover:bg-danger hover:text-white sm:h-[18px] sm:w-[18px] sm:min-h-0 sm:min-w-0 sm:opacity-0 sm:group-hover/img:opacity-100 sm:focus-within:opacity-100"
          >
      ...
      <button
        ...
        aria-label="Attach image"
        title="Attach image"
        className="flex h-12 w-12 min-h-11 min-w-11 shrink-0 items-center justify-center rounded-sm border border-dashed border-ink-700 text-ink-500 transition-colors hover:border-accent hover:text-accent-soft disabled:opacity-50"
      >
        <svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
