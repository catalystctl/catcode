...
export const Composer = forwardRef<ComposerHandle, Props>(function Composer(
  ...
  return (
    <div
      className={`composer-dock relative pb-[max(0.85rem,env(safe-area-inset-bottom))] pt-3 ${
    ...
    >
      <div className="mx-auto w-full max-w-[60rem]">
        ...
        {(activeFile || referencedFiles.length > 0) && (
          <div className="mb-2 flex min-w-0 flex-wrap items-center gap-1.5 px-0.5" aria-label="Prompt context">
            <span className="font-mono text-[10px] uppercase tracking-[0.12em] text-ink-500">Context</span>
            {activeFile && (
              <button
                type="button"
                onClick={() =>
                  setText((current) =>
                    referencedFiles.includes(activeFile)
                      ? current
        ...
        <div className="relative">
          ...
          >
            ...
            <textarea
              ...
              className="max-h-40 min-w-[8rem] flex-1 resize-none bg-transparent px-2.5 py-2 text-[16px] leading-relaxed text-ink-100 placeholder:text-ink-600 focus:outline-none disabled:opacity-50 sm:max-h-60 sm:text-[14px]"
            />
            {streaming ? (
              <div className="flex shrink-0 flex-wrap items-center justify-end gap-1 pb-0.5">
                {text.trim() && (
                  <>
                    <button
                      onClick={submit}
        ...
        <div
          className={`mt-2 items-center justify-between px-1 font-mono text-[10px] text-ink-500 ${
            compact ? "hidden" : "flex"
          }`}
        >
          <span className="flex min-w-0 items-center gap-1.5">
            <BoltIcon width={11} height={11} className="shrink-0 text-accent-soft" />
            <span className="truncate text-ink-400">{modelLabel}</span>
            <span className="text-ink-700">·</span>
            <span className="shrink-0">think: {thinkingLevel}</span>
            {flyoutOpen && (
              <>
                <span className="text-ink-600">·</span>
                <span className="text-accent-soft">
                  <kbd className={kbd}>↑↓</kbd> select <kbd className={`ml-1 ${kbd}`}>↵</kbd> confirm
                </span>
              </>
            )}
          </span>
          <span className="hidden shrink-0 sm:inline">
            {streaming ? (
              <>
                <kbd className={kbd}>Enter</kbd> queue · <kbd className={kbd}>Ctrl+↵</kbd> steer ·{" "}
                <kbd className={kbd}>Esc</kbd> {followUpQueued ? "clear queue" : "stop"}
              </>
            ) : (
              <>
                <kbd className={kbd}>Enter</kbd> send · <kbd className={kbd}>Shift+↵</kbd> newline ·{" "}
