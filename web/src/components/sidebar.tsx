      ...
      >
        ...
        <div className="px-2.5 pb-2">
          <div className="relative">
            <SearchIcon width={12} height={12} className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-ink-500" />
            <input
              value={query}
          ...
          {filtered.length === 0 ? (
            ...
            grouped.map((group) => (
              ...
              {group.sessions.map((s) => {
                ...
                return (
                  <li key={s.name} className="group relative mb-0.5">
                    <button
                      ref={(node) => {
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        {isRenaming ? (
                          ...
                          <>
                            <div className="line-clamp-2 pr-6 text-[12.5px] font-medium leading-snug text-ink-100" title={displayTitle}>
                              {displayTitle}
                            </div>
                            <div className="mt-0.5 flex items-center gap-1.5 font-mono text-[10px] leading-4 text-ink-500">
                              {showStreaming && (
                                <><span className="sr-only">Generating</span><span className="text-accent-soft">live</span><span>·</span></>
                              )}
                              {needsAttention && (
                                <span
                              ...
                              {s.messages != null && (
                                <>
                                  <span>·</span>
                                  <span>{s.messages} {s.messages === 1 ? "msg" : "msgs"}</span>
                                </>
                              )}
                            </div>
                          </>
