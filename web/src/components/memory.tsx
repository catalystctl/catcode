...
function scopeBadge(scope?: string) {
  if (scope === "global") {
    return { label: "global", Icon: GlobeIcon, cls: "bg-ink-800 text-accent-soft" };
  }
...
function MemoryCard({
  ...
  return (
    <div className="group rounded-sm border border-ink-800 bg-ink-900 px-3 py-2.5">
      {/* Title row */}
      <div className="flex items-start gap-2">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            ...
            {m.type && (
              <span className="rounded-sm bg-ink-800 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wide text-ink-400">
                {m.type}
              </span>
            )}
          </div>
      ...
      {content && (
        <div className="mt-1.5">
          <pre
            className={`whitespace-pre-wrap break-words rounded-sm border border-ink-800 bg-ink-950 p-2 font-mono text-[11.5px] leading-relaxed text-ink-300 ${
      ...
      <div className="mt-1.5 flex flex-wrap items-center gap-1">
        <span
          className={`inline-flex items-center gap-1 rounded-sm px-1.5 py-0.5 font-mono text-[10px] font-medium ${cls}`}
        >
        ...
        {m.tags &&
          m.tags
            .filter((t) => t !== m.type)
            .map((t) => (
              <span key={t} className="rounded-sm bg-ink-800 px-1.5 py-0.5 font-mono text-[10px] text-ink-400">
                {t}
              </span>
            ))}
      </div>
      ...
      <div className="modal-backdrop">
        ...
        >
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-y-auto p-4">
            {/* New memory */}
            <div className="mb-3 rounded-sm border border-ink-800 bg-ink-900 p-3">
              <textarea
                rows={3}
                value={text}
