...
export function OauthPromptBanner({ prompt, onSubmit, onDismiss }: Props) {
  ...
  return (
    <div className="my-3 overflow-hidden rounded-xl border border-accent/30 bg-accent/[0.04]">
      <div className="flex items-center gap-2 border-b border-accent/20 px-4 py-2.5">
        <LinkIcon width={15} height={15} className="text-accent-soft" />
        <span className="text-sm font-semibold text-ink-100">OAuth login required</span>
        <button
          onClick={onDismiss}
          className="ml-auto rounded-md p-1 text-ink-500 hover:bg-ink-800 hover:text-ink-100"
          aria-label="Dismiss"
        >
      ...
      <div className="px-4 py-3">
        {prompt.message && (
          <p className="mb-2 text-[12px] leading-relaxed text-ink-300">{prompt.message}</p>
        )}
        {prompt.url && (
          <a
            href={prompt.url}
            ...
            className="mb-3 block break-all rounded-lg border border-ink-800 bg-ink-950 p-2.5 font-mono text-[12px] leading-relaxed text-accent-soft transition-colors hover:bg-ink-900"
        ...
        {prompt.code && (
          <div className="mb-3 flex items-center gap-2 rounded-lg border border-ink-800 bg-ink-950 p-2.5">
            <span className="text-[10px] font-semibold uppercase tracking-wide text-ink-600">code</span>
            <code className="select-all font-mono text-[13px] font-medium text-ink-100">{prompt.code}</code>
            <button
              onClick={copyCode}
              className="ml-auto flex items-center gap-1 rounded-md px-1.5 py-1 text-[11px] text-ink-400 transition-colors hover:bg-ink-800 hover:text-ink-100"
              title="Copy code"
            >
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SendIcon width={14} height={14} /> Submit code
          </button>
          <button
            onClick={onDismiss}
            className="flex items-center gap-1.5 rounded-lg border border-ink-700 px-3.5 py-1.5 text-[13px] font-medium text-ink-300 transition-colors hover:border-ink-600 hover:bg-ink-850"
          >
            <XIcon width={14} height={14} /> Dismiss
          </button>
          <span className="ml-auto hidden text-[11px] text-ink-600 sm:inline">
            <kbd className="rounded bg-ink-800 px-1 py-0.5 font-mono text-[10px]">Enter</kbd> submit ·{" "}
            <kbd className="rounded bg-ink-800 px-1 py-0.5 font-mono text-[10px]">Esc</kbd> dismiss
          </span>
        </div>
      </div>
    </div>
